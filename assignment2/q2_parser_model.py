...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        pretrained_emb = tf.Variable(self.pretrained_embeddings)
        #pretrained_emb = self.pretrained_embeddings
        input_emb  = tf.nn.embedding_lookup(pretrained_emb,self.input_placeholder)
        embeddings = tf.reshape(input_emb,[-1,self.config.n_features*self.config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        W  = initializer((self.config.n_features*self.config.embed_size,self.config.hidden_size))
