...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        self.weigths = tf.Variable(np.zeros([self.config.n_features,self.config.n_classes]),dtype=tf.float32)
        self.biases  = tf.Variable(tf.expand_dims(tf.zeros(self.config.n_classes),0),dtype=tf.float32)
        pred = softmax(tf.add(tf.matmul(self.input_placeholder,self.weigths),self.biases))
