#!/home/oleks/anaconda3/bin/python
# -*- coding: utf-8 -*-
...
class StanfordSentiment:
    ...
    def sent_labels(self):
        ...
        dictionary = dict()
        phrases = 0
        with open(self.path + "/dictionary.txt", "r", encoding='latin1') as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
        ...
        labels = [0.0] * phrases
        with open(self.path + "/sentiment_labels.txt", "r", encoding='latin1') as f:
            first = True
            for line in f:
                if first:
                    first = False
        ...
        for i in range(self.numSentences()):
            sentence = [codecs.decode(word, 'latin1') for word in sentences[i]]
