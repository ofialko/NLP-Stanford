...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        sentence_ = [start]
        sentence_.extend(sentence)
        sentence_.extend([end])
        for i in range(len(labels)):
            tup = (sum(sentence_[i-window_size+1:i+window_size+2],[]),labels[i])
            windowed_data.append(tup)
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        pretrained_embedings = tf.Variable(self.pretrained_embeddings,dtype=tf.float32)
        embeddings = tf.nn.embedding_lookup(pretrained_embedings,self.input_placeholder)
