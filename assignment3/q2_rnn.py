...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if test >= max_length:
            ...
            sentence_ = [zero_vector]*max_length
            labels_   = [zero_label]*max_length
            sentence_[:test] = sentence
            labels_[:test]   = labels
            ret.append((sentence_,labels_,[True]*test + [False]*(max_length-test)))
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.nn.embedding_lookup(embeds,self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                drop_state = tf.nn.dropout(state, dropout_rate)
                y_t = tf.matmul(drop_state, U) + b2
                preds.append(y_t)
    ...
    def add_loss_op(self, preds):
        ...
        """
        # loss = tf.reduce_mean(tf.boolean_mask(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=preds,labels=self.labels_placeholder),
        #                 self.mask_placeholder))
